...
common() {
  ...
  # Get parent pom information (parent-pom.xml)
  local a_groupId=$(xmlstarlet sel -T -N x="http://maven.apache.org/POM/4.0.0" -t -v "/x:project/x:groupId" /project/parent-pom.xml)
  local a_artifactId=$(xmlstarlet sel -T -N x="http://maven.apache.org/POM/4.0.0" -t -v "/x:project/x:artifactId" /project/parent-pom.xml)
  local a_version=$(xmlstarlet sel -T -N x="http://maven.apache.org/POM/4.0.0" -t -v "/x:project/x:version" /project/parent-pom.xml)
  ...
  then
    ...
    then
      run_mvn install -q -f /project/parent-pom.xml
    else
      run_mvn install -f /project/parent-pom.xml
    fi
  fi
